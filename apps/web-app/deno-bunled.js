...
async function serveClient(req, basePath) {
    ...
    if (url.pathname.startsWith('/assets') || url.pathname.includes(basePath)) {
        let targetUrl = `https://gitlab.com/taocunguijs/deno-apps-dev/-/raw/master/dist/apps/cf-page${url.pathname}`;
        if (url.pathname.includes(basePath)) {
            targetUrl = `https://gitlab.com/taocunguijs/deno-apps-dev/-/raw/master/dist/apps/cf-page/index.html`;
        }
